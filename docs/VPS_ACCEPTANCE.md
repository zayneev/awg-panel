...
1. На VPS с AmneziaWG 5.20.x и отдельно с 5.21.2 запустите online-мастер из GitHub Release. Откажитесь от routing и убедитесь, что установлен только `/usr/local/bin/awgpanel`, а текущие peers, порты и nftables не изменились.
...
7. Остановите `awg-quick@awg0` и повторите обычную установку: она должна завершиться успешно с явным предупреждением. Несовместимая версия upstream 5.22.x должна остановить preflight до изменения файлов.
