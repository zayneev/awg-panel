...
- `G` во вкладке WARP — при установленных компонентах и выключенном routing зарегистрировать отдельное WARP-устройство с явным подтверждением условий Cloudflare и автоматической проверкой;
- `Esc` — назад или выход.
...
Первичную регистрацию можно выполнить в TUI: `W` → вкладка `WARP` → `G`. Панель отдельно запросит подтверждение условий Cloudflare, сохранит секреты с правами `0600` и автоматически выполнит health-check. Тот же сценарий доступен через CLI:
...
Импорт существующего стандартного wg-quick-конфига вместо регистрации:
